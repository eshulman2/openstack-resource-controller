...
func TestHandleAdminStateUpdate(t *testing.T) {
	testCases := []struct {
		...
		{name: "Identical true", newValue: true, existingValue: true, expectChange: false},
		{name: "Identical false", newValue: false, existingValue: false, expectChange: false},
		{name: "Different true to false", newValue: false, existingValue: true, expectChange: true},
		{name: "Different false to true", newValue: true, existingValue: false, expectChange: true},
	}

	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			resource := &orcv1alpha1.LoadBalancerResourceSpec{AdminStateUp: ptr.To(tt.newValue)}
